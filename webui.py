...
with shared.gradio_root as block:
    ...
    cfg_timer.tick(fn=get_cfg_timestamp, show_api=True, outputs=[cfg_timestamp])
        ...
        with gr.Column(scale=5):
            ...
            with gr.Row(elem_classes="type_row"):
                with gr.Column(scale=5):
                    ...
                    @prompt.input(show_api=True, inputs=prompt, outputs=spellcheck)
                    ...
                    @spellcheck.select(show_api=True, inputs=[prompt, spellcheck], outputs=prompt)
...
#                        ),
        ...
        run_button.click(fn=poke, show_api=True, inputs=run_event, outputs=run_event)

        def stop_clicked():
            worker.interrupt_ruined_processing = True
            shared.state["interrupted"] = False

        stop_button.click(fn=stop_clicked, show_api=True, queue=False)
        ...
        cfg_timestamp.change(fn=update_cfg, show_api=True, outputs=[cfg_timestamp] + state["cfg_items_obj"])
